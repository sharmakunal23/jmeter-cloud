...
/**
 * Populates request-scoped MDC keys — {@code blobId} from the path and
 * {@code actor} from the {@code X-Actor} header — so every log line emitted
 * during a blob operation carries them.
 *
 * <p>Scoped by {@link CriticalPaths}, so actuator and swagger requests are
 * skipped.
 */
@Component
