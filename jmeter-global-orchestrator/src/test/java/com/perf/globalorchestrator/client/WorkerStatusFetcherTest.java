...
@DisplayName("WorkerStatusFetcher — one relay batch per region, declared workers direct, nothing relayed to a region known down")
class WorkerStatusFetcherTest {
    ...
    void routesAndBatches() {
        RegionRegistry regions = TestRegions.registryOf("na-east=" + URL);
        ...
        // A declared worker in the SAME cluster: hub-reachable address, so the
        // relay batch cannot serve it — fetched directly (CLUSTER-CAPACITY).
        WorkerRef declared = new WorkerRef("na-east", "lab-1", "http://lab-1.example.test:8080");
        when(regional.statusBatch(URL, List.of("e-1", "e-2"))).thenReturn(Map.of(
                "e-1", Optional.of(Map.of("state", "RUNNING")),
                "e-2", Optional.empty()));
        when(direct.getTestStatus(declared)).thenReturn(Optional.of(Map.of("state", "COMPLETED")));

        Map<String, Map<String, Object>> out = new WorkerStatusFetcher(direct, regional, regions, new RegionRouter(regions))
                .fetch(List.of(e1, e2, declared));
    ...
    @Test
    void skipsRelayForARegionKnownToBeDownButStillAsksDeclaredWorkersDirectly() {
        RegionRegistry regions = TestRegions.registryOf("na-east=" + URL);
        regions.markReachable("na-east", new RegionCapabilities("na-east", "jmeter-cloud", "workers", "img", 8080, "dev"));
        ...
        RegionalClient regional = mock(RegionalClient.class);
        WorkerRef declared = new WorkerRef("na-east", "lab-1", "http://lab-1.example.test:8080");
        when(direct.getTestStatus(declared)).thenReturn(Optional.of(Map.of("state", "RUNNING")));
        WorkerStatusFetcher fetcher = new WorkerStatusFetcher(direct, regional, regions, new RegionRouter(regions));

        Map<String, Map<String, Object>> out = fetcher.fetch(List.of(
                new WorkerRef("na-east", "e-1", "http://e-1.workers:8080"), declared));

        assertThat(out).containsOnlyKeys("lab-1");   // spun worker skipped, declared still answered
    ...
    void aFailedBatchIsNoAnswersNotAnError() {
        ...
        WorkerStatusFetcher fetcher = new WorkerStatusFetcher(mock(LocalOrchestratorClient.class), regional,
                regions, new RegionRouter(regions));
