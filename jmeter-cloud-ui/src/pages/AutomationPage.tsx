...
/**
 * Automation — every schedule the platform fires, in three sections:
 * <b>Workflow automation</b> (run a workflow), <b>Platform reports</b> (email a
 * report) and <b>Platform infrastructure</b> (scale a cluster out or in).
 *
  ...
  return (
    <section className="automationPage">
      ...
      <SchedulesSection
        ...
        empty={<>No workflow schedules. Add one to run a <Link to="/workflows">workflow</Link> on a cadence — a
               one-step workflow is how you schedule a single load test.</>}
        jobs={workflowJobs}
        columns={[
        ...
        fireLabel="Run now"
        fireBody={<>This starts the workflow immediately, in addition to its normal cadence. It is skipped if
                  that workflow is already running or its graph no longer fits the group&apos;s reservation.</>}
        deleteBody={<>This removes the schedule. The workflow itself is untouched and can still be run by hand.</>}
      ...
      <SchedulesSection
        ...
        empty={<>No scaling schedules. Pair a scale-in in the evening with a scale-out in the morning to stop
               paying for idle workers overnight.</>}
        jobs={infraJobs}
        columns={[
          { header: "Group", cell: (j) => groupName(j.groupId) },
        ...
        fireLabel="Apply now"
        fireBody={<>This scales the cluster immediately, in addition to its normal cadence. Workers in use are
                  never released.</>}
