...
per second to `POST /api/v1/ingest`, which lands it in the
`jmetercloud_metrics` Postgres database and folds it into the rollup tables
the orchestrators read. Ingest is idempotent on
`(runId, workerId, label, windowSecond)`, so a worker replaying its disk
buffer after an outage is always safe.
